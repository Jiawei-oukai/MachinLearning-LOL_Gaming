...
from ChampionNameMappingGold import ChampionNameMappingGold
...
def to_comparisonFeatureData_gold(df,stage):
    dt_columns = ChampionNameMappingGold.dt_feature_columns()
    df_DT = pd.DataFrame(columns=dt_columns)
    ...
    model = ChampionNameMappingGold(stage)
    for index, row in tqdm(df_new.iterrows()):
        ids = row.values.tolist()
        model.fit_data_id(ids)
        dtv = model.dt_feature_values()
...
def to_allFeatureData_gold(df,stage):
    all_columns = ChampionNameMappingGold.all_feature_columns()
    df_all = pd.DataFrame(columns=all_columns)
